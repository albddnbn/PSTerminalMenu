...
#>
    ...
    Check-CreateFile -Path "$env:PSMENU_DIR\reports\$thedate\$REPORT_DIRECTORY" -ItemType 'directory'
...
'@
    ...
    Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] :: Outputting BIOS info for: " -nonewline
    Write-Host "$((($initial_bios_info).pscomputername) -join ', ')" -nonewline -foregroundcolor Green
    Write-Host "."

    if ($outputfile.Tolower() -ne 'n') {
        $initial_bios_filepath = "$env:PSMENU_DIR\reports\$thedate\$REPORT_DIRECTORY\initial-bios-info $outputfile"
        Output-Reports -Filepath $initial_bios_filepath -Content $initial_bios_info -ReportTitle 'InitialBios' -XlsxFile $true
    }
    else {
        $initial_bios_info | out-gridview
    }
    ...
    $update_bios_scriptblock = {
        ...
        if ($biosexe) {
            ...
            Start-Process "$($biosexe.fullname)" -ArgumentList "/s /l=$TargetDIR\biosupdatelog.txt /p=border9" -Wait
    ...
    Check-CreateFile -Path "$env:PSMENU_DIR\reports\$thedate\$REPORT_DIRECTORY\logs" -Itemtype 'directory'

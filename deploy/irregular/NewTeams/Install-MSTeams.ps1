...
function CreateScheduledTask {
    ...
    $ActiveUser = Get-process -name 'explorer' -includeusername -erroraction silentlycontinue | select -exp username
...
function Invoke-FileCertVerification {
    ...
    if ($Cert) {
        ...
        if ($cert.Subject -match "O=Microsoft Corporation" -and $CertStatus -eq "Valid") {
            ...
            # IF RootCert = "" or null, [string]::isnullorempty returns true / abuddenb
            if (-not [string ]::IsNullOrEmpty($RootCert)) {
                # if $RootCert has a value - code will execute here / abuddenb
                #Verify root certificate exists in local Root Store
                # Cert:\ is a UNC path that provides access to Root certificate store on local machine / abuddenb
                # $PSItem is another way of doing $_ when piping arraylist into  where-object / abuddenb

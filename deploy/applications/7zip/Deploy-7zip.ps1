...
Try {
    ...
    [string]$appScriptAuthor = 'Jason Bergner' # edited: abuddenb
    ##*===============================================
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        Show-InstallationProgress -StatusMessage "Removing Any Existing Version of 7-Zip. Please Wait..."
        ...
        ## Install 7-Zip (64-bit)
        $ExePath64 = Get-ChildItem -Path "$dirFiles\x64" -Include "7z*x64.exe" -File -Recurse -ErrorAction SilentlyContinue
        $MsiPath64 = Get-ChildItem -Path "$dirFiles\x64" -Include "7z*x64.msi" -File -Recurse -ErrorAction SilentlyContinue
        $Transform64 = Get-ChildItem -Path "$dirFiles\x64" -Include "*.mst" -File -Recurse -ErrorAction SilentlyContinue

        If ($ExePath64.Exists) {
            Write-Log -Message "Found $($ExePath64.FullName), now attempting to install $appName."
            Show-InstallationProgress "Installing 7-Zip (64-bit). This may take some time. Please wait..."
            Execute-Process -Path "$ExePath64" -Parameters "/S" -WindowStyle Hidden
            Start-Sleep -Seconds 5
        }

        # ElseIf (($MsiPath64.Exists) -and ($Transform64.Exists)) {
        #     Write-Log -Message "Found $($MsiPath64.FullName) and $($Transform64.FullName), now attempting to install $appName."
        #     Show-InstallationProgress "Installing 7-Zip (64-bit). This may take some time. Please wait..."
        #     Execute-MSI -Action Install -Path "$MsiPath64" -AddParameters "TRANSFORMS=$Transform64"
        # }

        ElseIf ($MsiPath64.Exists) {
            Write-Log -Message "Found $($MsiPath64.FullName), now attempting to install $appName."
            Show-InstallationProgress "Installing 7-Zip (64-bit). This may take some time. Please wait..."
            Execute-MSI -Action Install -Path "$MsiPath64"
        }

		# abudddenb: commented out this part --> one of the things it does is create a 7zip icon for all associated filetypes (.zip) - may confuse users
        ## Configure 7-Zip File Associations (64-bit)
        # $FileAssoc = Get-ChildItem -Path "$dirSupportFiles" -Include 7zipFileAssoc.reg -File -Recurse -ErrorAction SilentlyContinue
        # If($FileAssoc){
        # Write-Log -Message "Configuring $appName File Associations (64-bit)"
        # cmd /c regedit.exe /s $FileAssoc
        # }

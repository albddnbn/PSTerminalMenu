Function Apply-DellCommandUpdates {
    ...
    .PARAMETER EncryptedPasswordFile
        Path to encrypted password file. Password must be encrypted with the key in the EncryptedPasswordFileKey parameter.
        Function will search for the file and copy to target computers when applying updates.
        Create an encrypted password file using dcu-cli.exe: dcu-cli.exe /generateEncryptedPassword -encryptionKey=<inline value> -password=<inlinevalue> -outputPath=<folderpath>
    ...
    .PARAMETER EncryptedPasswordFileKey
        Key used to encrypt the password file when it was created.
        Create an encrypted password file using dcu-cli.exe: dcu-cli.exe /generateEncryptedPassword -encryptionKey=<inline value> -password=<inlinevalue> -outputPath=<folderpath>
        https://www.dell.com/support/manuals/en-us/command-update/dellcommandupdate_rg/dell-command-%7C-update-cli-commands?guid=guid-92619086-5f7c-4a05-bce2-0d560c15e8ed&lang=en-us
    ...
    param(
        ...
        [string]$EncryptedPasswordFileKey
    )
    ...
    BEGIN {
        ...
        ## 3. Make sure the encrypted password file exists and is accessible.
        if (Test-Path $EncryptedPasswordFile -ErrorAction SilentlyContinue) {
            $EncryptedPasswordFile = Get-Item $EncryptedPasswordFile
            $EncryptedPasswordFileName = $EncryptedPasswordFile.name
            Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] :: Found encrypted password file at: $($EncryptedPasswordFile.fullname)." -Foregroundcolor Yellow
        }
        else {
            Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] :: Encrypted password file not found, skipping." -Foregroundcolor Yellow
            return
        }
        ...
        ForEach ($single_computer in $TargetComputer) {
            ...
            if ($single_computer) {
                ...
                if ($pingreply) {
                    ...
                    $single_session = New-PSSession $single_computer -ErrorAction SilentlyContinue
                    if ($single_session) {
                        Copy-Item -Path "$($encryptedpasswordfile.fullname)" -Destination "C:\temp\" -Force -ToSession $single_session


                        ## 3. If machine was responsive, check for dell command update executable and apply updates, report on results.
                        $dcu_apply_result = Invoke-Command -ComputerName $single_computer -scriptblock {

                            $encryptedfile = $using:EncryptedPasswordFileName
                            $encryptedfile = Get-Item "C:\temp\$encryptedfile"

                            $encryptionkey = $using:EncryptedPasswordFileKey
                            ...
                            ## dcu-cli /applyUpdates -encryptedPasswordFile=C:\Temp\EncryptedPassword.txt -encryptionKey="myEncryptionKey"
                            &$($dellcommandexe.fullname) /applyUpdates -encryptedPasswordFile=$encryptedfile -encryptionKey="$encryptionkey" -updateType="bios,firmware" -reboot=enable -silent

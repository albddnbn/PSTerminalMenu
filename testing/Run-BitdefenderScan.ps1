function Run-BitdefenderScan {
    # C:\Program Files\Bitdefender\Endpoint Security
    # ./product.console.exe /c FileScan.OnDemand.RunScanTask custom path="C:\Users\Public"
    <#
    .SYNOPSIS
        Targets Bitdefender command-line executable on local system and kicks off virus scan for target folder(s).

    .DESCRIPTION
        Bitdefender Endpoint Security Tools remote scan.
    ...
    param (
        ...
        [String]$Targetfolders, # comma-separated list of target folders:
        [string]$Outputfile = ''
    )
    ...
    BEGIN {
        ...
        if ($TArgetfolders -like "*,*") {
            $TargetFolders = $Targetfolders.split(',')
        } else {
            ...
            if ($single_computer) {
                ...
                if (Test-Connection $single_computer -count 1 -quiet) {
                    ...
                    $single_results = Invoke-Command -ComputerName $single_computer -ScriptBlock {
                        ...
                        ## will only check for product.console.exe to confirm installation for now.
                        try {
                            ## get scan executable:
                            ...
                            Write-Host "BF Not installed or something went wrong with scan."
                        }
                    }
                    ## ---------------------------------------------------------------------------
                    $results.add($single_results)

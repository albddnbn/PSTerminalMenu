...
function Scan-Inventory {
    ...
    Function Intake-ScannedItems {
        param(
            [Parameter(ValueFromPipeline = $true, Mandatory = $true)]
            [String[]]$scanned_codes
            # $INVENTORY_LIST
        )
        BEGIN {
            Write-Host "Begin block - item intake."
        ...
        END {
            Write-Host "End block - item intake."
        }
    }
    ...
    ## If Scanlist was submitted - pipe to Intake function
    if ($ScanList) {
        Write-Host "Scan list detected, getting content and piping into Intake-ScannedItems function.."
        $scanned_codes = Get-Content $ScanList

        $scanned_codes | Intake-ScannedItems
    }
    ## If no scanlist - user will be prompted to scan codes in one by one
    else {
        Write-Host "No scan list detected, starting scan loop.."
        while ($true) {
            $user_input = Read-Host "Scan code, or type 'exit' to quit"
            if ($user_input.ToLower() -eq 'exit') {
                break
            }
            $user_input | Intake-ScannedItems
        }
    }
    ...
    $INVENTORY_LIST = [System.collections.arraylist]::new()
    ...
    $INVENTORY_LIST | Export-CSV "$outputfile.csv" -NoTypeInformation -Force
    ...
    $xlsx = $INVENTORY_LIST | Export-Excel @params

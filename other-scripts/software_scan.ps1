##############################################################################
#.SYNOPSIS
# Does an inventory of the systems in a text file list
#
#.DESCRIPTION
# This script will allow you to get the full installed list of all
# applications that the local registry of the machine is aware of.
#
#.PARAMETER File
# The txt input file that contains a list of fully qualified dns names of the  
# windows systems you want to inventory.
#
#.PARAMETER Outfile
# The output file you want to use. Be sure to use the correct extension
#
#.PARAMETER html
# html output format for the outfile.
#
#.PARAMETER csv
# csv output format for the outfile.
#
#.EXAMPLE
# software_inventory.ps1 -f .\cad2019_rooms.txt.txt -o .\cad2019_inventroy_8-12-19.csv -c
# Initiating Software inventory
# Performing Inventory de Software on your computer s-b212-01.dtcc.edu
# Performing Inventory de Software on your computer s-b228-01.dtcc.edu
# Performing Inventory de Software on your computer s-b228-02.dtcc.edu
#
# software_inventory.ps1 -f .\cad2019_rooms.txt.txt -output .\cad2019_inventroy_8-12-19.html -h
# Initiating Software inventory
# Performing Inventory de Software on your computer s-b212-01.dtcc.edu
# Performing Inventory de Software on your computer s-b228-01.dtcc.edu
# Performing Inventory de Software on your computer s-b228-02.dtcc.edu
...
function pc-health {
    ...
    { Write-Host " $check is available " }
    else {
        Write-Host " $check is not available "
        continue 
    }
 
}
...
#Get-Content C:\Users\ahamil10\PSHELL\Computers.txt | SoftwareInvent | Select-Object Computer, Software, Manufacture, Version | ConvertTo-HTML -head $a | Out-File C:\Users\ahamil10\PSHELL\Computers.html | Set-Content -Encoding UTF8  



#$workit = get-content $File

If ($html) {
    Get-Content $File | SoftwareInvent | Select-Object Computer, Software, Manufacture, Version | ConvertTo-HTML -head $a | Select-String -Pattern 'Update', 'Microsoft' -CaseSensitive -NotMatch | Out-File $outfile | Set-Content -Encoding UTF8  
    write-Host "Inventory successfully held" -ForegroundColor Yellow
}

If ($csv) {
    Get-Content $File | SoftwareInvent | Select-Object Computer, Software, Manufacture, Version | ConvertTo-Csv -NoTypeInformation | Select-String -Pattern 'Update', 'Microsoft' -CaseSensitive -NotMatch | Out-File $outfile | Set-Content -Encoding UTF8  
    write-Host "Inventory successfully held" -ForegroundColor Yellow
}

If ($everything) {
    Get-Content $File | SoftwareInvent | Select-Object Computer, Software, Manufacture, Version | ConvertTo-Csv -NoTypeInformation | Out-File $outfile | Set-Content -Encoding UTF8  
    write-Host "Inventory successfully for all Software on pc" -ForegroundColor Yellow
}

If ($csv -eq $html ) {

    Write-host -ForegroundColor Red "Choose h for html output or c for csv output"

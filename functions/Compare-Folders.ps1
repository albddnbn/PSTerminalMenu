function Compare-Folders {
    <#
    .SYNOPSIS
        Compares target folders using Compare-Object, output is a bit easier to read than Compare-Object.
    ...
    .PARAMETER ReferenceFolder
        Reference Object is created using Get-Childitem -Path ReferenceFolder -Recurse

    .PARAMETER DifferenceFolder
        Difference Object is created using Get-Childitem -Path ReferenceFolder -Recurse
    ...
    .NOTES
        ...
        Comparisons right now just take actual FILE PRESENCE into account.
        Future plans are to include comparisons of file size, last write time, etc. - made into easy to read output.
    #>
    [CmdletBinding()]
    param (
        [Parameter(
    ...
    PROCESS {
        ForEach ($single_computer in $TargetComputer) {
            ...
            if ($single_computer) {
                ...
                if (Test-Connection $single_computer -Count 1 -Quiet -ErrorAction SilentlyContinue) {

                    $comparison_results = Invoke-Command -ComputerName $single_computer -SCriptblock {
                        ## ---------------------------------------------------------------------------
                        ...
                        $obj = [pscustomobject]@{
                            ...
                            ReferenceFolder          = $Reference_obj
                            DifferenceFolder         = $difference_obj
                            FilesPresentInReference  = $($files_present_in_ref -join ',')
                            FilesPresentInDifference = $($files_present_in_diff -join ',')
                        }
                    ...
                    if ($comparison_results) {
                        $results.Add($comparison_results) | Out-Null
                    }
                    else {
                        Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] :: $single_computer returned no results." -Foregroundcolor Yellow
                    }
                }
                else {

function Scan-SoftwareInventory {
    ...
    END {
        ...
        $Announcement = "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] :: Scan-SoftwareInventoryJob for $($TargetComputer -join ', ') completed.`nOutput file location(s): $outputfile.csv, $outputfile.xlsx"
        $Announcement | Out-File "$env:PSMENU_DIR\completedjobs\Scan-SoftwareInventoryJob--$(Get-Date -Format 'yyyy-MM-dd').txt" -Append
